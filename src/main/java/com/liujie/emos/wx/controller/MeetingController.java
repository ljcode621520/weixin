...
import com.liujie.emos.wx.db.pojo.TbMeeting;
import com.liujie.emos.wx.exception.EmosException;
import com.liujie.emos.wx.service.MeetingService;
...
public class MeetingController {
    ...
    public R searchMyMeetingListByPage(@Valid @RequestBody MeetingSearchMyListByPageForm form, @RequestHeader("token") String token){
    ...
    public R insertMeeting(@Valid @RequestBody MeetingInsertForm form, @RequestHeader("token") String token){
    ...
    public R searchMeetingById(@Valid @RequestBody MeetingSearchByIdFrom form){
    ...
    public R updateMeetingInfo(@Valid @RequestBody MeetingUpdateInfoForm form){
        if(form.getType()==2&&(form.getPlace()==null||form.getPlace().length()==0)){
            throw new EmosException("线下会议地点不能为空");
        }
        DateTime d1= DateUtil.parse(form.getDate()+" "+form.getStart()+":00");
    ...
    public R deleteMeetingById(@Valid @RequestBody MeetingDeleteByIdForm form){
    ...
    public R recieveNotify(@Valid @RequestBody WorkFlowRecieveNotifyForm form){
    ...
    public R searchRoomIdByUUID(@Valid @RequestBody MeetingSearchRoomIdByUUIDForm form){
    ...
    public R searchUserMeetingInMonth(@Valid @RequestBody MeetingSearchUserInMonthForm form, @RequestHeader("token") String token){
        int userId=jwtUtil.getUserId(token);
        HashMap param=new HashMap();
